...
const model_3 = require("../affiliateTransaction/model");
const model_4 = __importDefault(require("../product/model"));
const calculateAffiliateCommission = (productId, affiliateCode, orderAmount, orderId) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        // Find affiliate user
        const user = yield model_2.default.findOne({
        ...
        if (!user)
            return null;
        // Find product
        const product = yield model_4.default.findById(productId);
        if (!product || !product.affiliateEnabled)
            return null;
        // Use hardcoded max limit since settings are removed
        const maxLimit = 1000;
        ...
        const commission = new model_1.AffiliateCommission({
            affiliate: user._id,
            product: productId,
            order: orderId,
            commissionAmount,
            ...
            status: "pending", // Always pending since auto-approval settings removed
        });
        ...
        const transaction = new model_3.AffiliateTransaction({
            affiliate: user._id,
            product: productId,
            order: orderId,
            commissionAmount,
