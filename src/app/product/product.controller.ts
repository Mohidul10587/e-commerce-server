...
export async function getProductBySlug(req: Request, res: Response) {
  try {
    ...
    if (!product || product.isTrashed) return res.status(404).json({ message: "Product not found" });
...
export async function updateProduct(req: Request, res: Response) {
  try {
    ...
    if (productData.slug) {
      const slugExists = await prisma.product.findFirst({ where: { slug: productData.slug, NOT: { id } } });
      if (slugExists) return res.status(409).json({ message: "Slug already exists" });
    }

    const newVariantSkus = variants.filter((v) => !v.id).map((v) => v.sku);
    if (newVariantSkus.length > 0) {
      const duplicateSku = await prisma.productVariant.findFirst({ where: { sku: { in: newVariantSkus } } });
      if (duplicateSku) return res.status(409).json({ message: `SKU already exists: ${duplicateSku.sku}` });
    }

    const product = await prisma.$transaction(async (tx) => {
      await tx.product.update({ where: { id }, data: { ...productData, keywords: productData.keywords ?? [] } });
...
export async function updateVariantStock(req: Request, res: Response) {
  try {
    ...
    const { action, quantity, note } = req.body as { action: StockAction; quantity: number; note?: string };
...
export async function getStockHistory(req: Request, res: Response) {
  try {
    const variantId = parseInt(req.params.variantId);
    const history = await prisma.stockHistory.findMany({ where: { variantId }, orderBy: { createdAt: "desc" } });
